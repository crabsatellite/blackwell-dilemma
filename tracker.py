...
def generate_report(series, write=True):
    ...
    lines = ["<!-- GENERATED by tracker.py --report; DO NOT EDIT. -->"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--check-report', action='store_true',
                        help='Fail if prediction_status.md is stale')
    ...
    if args.report or args.check_report:
        if SNAPSHOTS_FILE.exists():
            ...
            if args.check_report:
                expected = generate_report(series, write=False)
                report_path = BASE_DIR / "prediction_status.md"
                current = report_path.read_text(encoding='utf-8') if report_path.exists() else ""
                stale = int(current != expected)
                print(f"prediction_status_stale={stale}")
                sys.exit(stale)
