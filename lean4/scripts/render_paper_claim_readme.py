...
CLAIM_LINEAGE = REPO_ROOT / "paper" / "claim_lineage.json"
CITATION_INVENTORY = REPO_ROOT / "paper" / "citation_inventory.json"
...
def claim_lineage_summary() -> dict[str, object]:
    lineage = json.loads(CLAIM_LINEAGE.read_text(encoding="utf-8"))
    relations = Counter(item["relation"] for item in lineage["claims"])
    return {
        "preserved": len(lineage["claims"]),
...
def render_lean_readme(
    ...
    citation_summary: dict[str, object],
) -> str:
...
research_claims_preserved={lineage_summary['preserved']}/{len(claims)}
claim_lineage_relations={rendered_counts(lineage_summary['relations'])}
...
manuscript_citation_keys={citation_summary['citation_keys']}
bibliography_entries={citation_summary['bibliography_entries']}
critical_reference_bindings={citation_summary['critical_reference_bindings']}
```
...
def main() -> int:
    ...
    lineage_summary = claim_lineage_summary()
    citation_inventory = json.loads(CITATION_INVENTORY.read_text(encoding="utf-8"))
    if citation_inventory.get("manuscript_sha256") != inventory.get("manuscript_sha256"):
        raise RuntimeError("citation inventory does not match manuscript inventory")
    citation_summary = citation_inventory["summary"]
    ...
    lean_text = render_lean_readme(
        claims, metadata, root_imports, publication_summary, lineage_summary,
        citation_summary
    )
    required_artifacts = [
        ...
        REPO_ROOT / "paper" / "citation_inventory.json",
    ...
    root_text = render_root_readme(
        claims, root_imports, publication_summary, lineage_summary,
        citation_summary
    )
