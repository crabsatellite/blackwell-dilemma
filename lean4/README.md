...
`#print axioms` entry, and that every `*_current` theorem in the paper
semantic gate, plus every paper-semantic gate `def`, is covered by the same
audit. The same coverage rule applies to every paper-semantic gate
`structure` and `inductive` declaration.
