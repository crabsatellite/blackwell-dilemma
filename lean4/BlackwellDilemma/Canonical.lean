...
theorem Phi_B_monotone {β₁ β₂ : ℝ} (hβ₁ : 0 < β₁) (h_le : β₁ ≤ β₂) :
    Phi_B β₁ ≤ Phi_B β₂ := by
  unfold Phi_B
...
/-- `Φ_B β → 1` as `β → ∞`: the argument `Δ_B/√(2σ²(β)) → ∞` because
    ...
    paper source: line 825 ("`Φ_B(β)` ranges over `(1/2, 1)`"). -/
theorem Phi_B_tendsto_one_atTop :
...
/-! The former cognitive-threshold bridge in this historical module was
removed because it derived paper claims from the constant `kappaAgent = 1/2`
carrier and renamed an unrelated minimizer as an inflection point. The exact
paper-facing experiment now lives in `UnifiedCognitiveFiveState.lean`. -/
/-! The historical Bayesian-naive bridge was removed because it used the wrong continuation value, produced the false 2/3 threshold, and encoded an above-threshold reversal absent from the strict prior-only paper model. The paper-facing proof now lives in `UnifiedBayesianNaiveFiveState.lean`. -/
