...
def entry_prop_topo_cluster : GapEntry where
  ...
  obstacleOrAttribution :=
    "R23-C1 PARTIAL: Cat 3 atomic structural-equation `expectedTopoLoss_conditional_def` (`= n/(n+1) − k/(k+1)`) hosts the paper-stated order-statistics decomposition (paper line 292); Cat 1 derived theorem `gap_topo_cluster_relation` closes the closed-form `(n−k)/((n+1)(k+1))` algebraically via `field_simp; ring`. The Cat 2 dependency on David & Nagaraja 2003 Eq. 2.1.4 remains a docstring-only acknowledgement; full closure requires Mathlib product-measure infrastructure. Regime asymptotics — `gap_topo_loss_below_threshold_OPEN` and `gap_topo_loss_above_threshold_OPEN` — remain BLOCKED as separate entries. R24-D: AxiomAudit now instruments the derived closure; output `[propext, expectedTopoLoss_conditional, expectedTopoLoss_conditional_def, Classical.choice, Quot.sound]` matches the documented Cat 1-derives-from-Cat 3-atom chain."
  conditionalOn := []
...
def entry_topo_loss_above : GapEntry where
  ...
  obstacleOrAttribution := "Cat 2 / Cat 3 edge-case axiom accepted on Paper Thm 3.3 + Grimmett 1999 _Percolation_ 2nd ed. (Springer) authority. Encoded via opaque carrier `expectedTopoLoss : ℕ → ℝ → ℝ` (paper-novel Cat 3 carrier above threshold); the substantive two-sided bound is a Cat 2 dependency on Grimmett 1999 (Mathlib lacks formalized Z² lattice + percolation-measure machinery, same gap as `entry_topo_loss_below` and `entry_harris_kesten`). R26: BLOCKED-def encoding retired in favour of plain Cat 3 axiom with dual Cat 2 / Cat 3 dependency acknowledged in docstring."
  conditionalOn := []
...
def entry_lem_wrongness : GapEntry where
  ...
  obstacleOrAttribution :=
    "Substantive proof requires bounded-convergence + Φ-tail integral machinery not in Mathlib. R21-A: the two R4-deferred paper-faithful antecedents have been landed — `DegreeTwoStartingVertex` premise (paper `|N_R(v_0)| = 2`) and whole-family topology-blindness `∀ β, IsTopologyBlind (signalFamily β)` (paper `{π_β}_β` family-level scope). The signature now matches Lemma `lem:wrongness` line 338 verbatim on hypotheses; the OPEN status reflects the still-missing Mathlib bounded-convergence + Φ-tail integral substantive-proof machinery, not signature drift."
  conditionalOn := []
...
def entry_prop_threshold_alpha : GapEntry where
  name := "gap_threshold_alpha_monotone"
  status := GapStatus.gapOpen
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  paperSource := "Proposition prop:threshold-alpha (re-exports cognitive_threshold_part5_OPEN)"
  attackHistory :=
    [ "R1 2026-05-12: re-export of `gap_cognitive_threshold_part5_OPEN` (Cat 3 chain).",
      ...
      "R24-B 2026-05-13: REVERTED CLOSED → OPEN per R23-D Audit 1+2+3 α-erasure violation. The R23-C2 Cat 1 promotion was a tautological closure (Pattern 4: tautological-premise) — `rw [kappaStar_def, kappaStar_def]` collapsed both sides to the SAME α-free expression, discharging `≤` because the two sides were definitionally EQUAL (independent of `α₁ ≤ α₂` premise). Paper-source re-verification: `m(κ)` (paper line 489 + 505) is literally α-free (depends on (p, κ) only via posterior estimates of dynamic value), so paper's Part 3 inf-formula `κ* = inf{κ > 0 : m(κ) ≥ 0}` is itself α-free. The α-monotonicity of `κ*` enters via a DIFFERENT characterisation: paper Prop:threshold-alpha proof line 540 reads `The threshold κ* is the value of κ where the welfare transitions from non-monotone to monotone in β. Since higher α increases the trap probability ... a higher κ is needed to compensate: ∂κ*/∂α > 0`. Paper does NOT prove the inf-formula is α-monotone; it derives α-monotonicity from the welfare-transition characterisation via the trap-probability argument. Therefore: under the current `kappaStar_def` encoding (which faithfully reflects ONLY the Part 3 inf-formula), Part 5 is NOT derivable from `kappaStar_def` alone. R24-B Option B chosen (Option A rejected — refactoring `mean_estimate_gap` to take α would phantom-introduce an α-dependence the paper does not state on `m(κ)` itself). Restored to atomic Cat 3 OPEN axiom `gap_cognitive_threshold_part5_OPEN`; wrapper `gap_threshold_alpha_monotone` re-exports the OPEN axiom; inputCategory Cat 1 → Cat 3. The substantive paper claim now awaits encoding via a SEPARATE atomic Cat 3 axiom for the welfare-transition α-monotonicity (independent of `kappaStar_def`); future-round candidate.",
      "R27-A 2026-05-13: Cat 3 sub-classification WORKING_ASSUMPTION per `feedback_gap_ledger_in_lean4` 2026-05-13 extension; status retained as OPEN (higher-level paper claim pending derivation from Cat 1 + Cat 2 + Cat 3 atomic inputs — 必须 close per discipline). New `subClass` field set to WORKING_ASSUMPTION." ]
  scope := "Proposition prop:threshold-alpha (re-exports cognitive_threshold_part5_OPEN)"
  obstacleOrAttribution := "OPEN axiom; wrapper re-exports `gap_cognitive_threshold_part5_OPEN`. R24-B audit: paper's `m(κ)` is α-free, so the Part 3 inf-formula `kappaStar_def` cannot encode α-monotonicity; the paper proves α-monotonicity via a SEPARATE welfare-transition characterisation (line 540). Honest encoding: independent atomic Cat 3 axiom for Part 5, not derivable from `kappaStar_def`. Refactoring `mean_estimate_gap` to depend on α would phantom-introduce paper-unsupported structure (Cat 3 phantom-attribution antipattern). Future-round candidate: add a separate atom encoding the welfare-transition α-monotonicity (e.g., Cat 3 axiom on the `agentWelfare`/trap-probability dependence of `kappaStar` via the Prop:threshold-alpha proof argument)."
  conditionalOn := []
...
def entry_prop_interior_optimum : GapEntry where
  name := "FiveState.L + gap_interior_optimum_OPEN"
  status := GapStatus.gapPartial
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  ...
  obstacleOrAttribution := "PARTIAL bundle: closed-form L β p def Cat 3 CLOSED; existence of unique interior minimum (β* ≈ 1.5) remains OPEN (numerical fact requiring continuous-function-on-compact-interval Mathlib infrastructure plus uniqueness derivation)."
  conditionalOn := []

def entry_prop_three_regime : GapEntry where
  name := "gap_three_regime_{reversal_{existence,uniqueness,nonmonotone,overshoot_decreasing,overshoot_continuous,overshoot_vanishes_at_p1}_OPEN,cognitive_augmentation_{arithmetic_part,monotonicity},sufficient_cognition} + opaque carrier betaStarOfP"
  status := GapStatus.gapPartial
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  ...
  obstacleOrAttribution :=
    "PARTIAL bundle: (a) `gap_three_regime_cognitive_augmentation_arithmetic_part` CLOSED Cat 1 (R17-C split via `unfold + linarith`); (b) `gap_three_regime_cognitive_augmentation_monotonicity` CLOSED Cat 1 (R22-A promotion via `L_monotone_under_q_le_5_9` + 5 new Phi helpers); (c) `gap_three_regime_sufficient_cognition` CLOSED Cat 1 (R22-A same shared lemma); (d) reversal sub-axioms (6: existence, uniqueness, nonmonotone, overshoot_decreasing, overshoot_continuous, overshoot_vanishes_at_p1) all OPEN — these are paper-novel claims about Regime (i) `p ∈ [0, p_1)` requiring genuine paper-substantive content (existence of interior minima, uniqueness from unimodal structure, envelope differentiation). R22-A CLOSED 2 of the previously 8 OPEN axioms in this bundle (3 sub-claims now CLOSED, 6 sub-claims remain OPEN); the inputCategory tag remains Cat 3 dominant per the bundle's reversal-regime content. R22-A introduces 5 new Cat 1 helpers in ClassicalResults.lean (`Phi_strictMono`, `Phi_monotone`, `Phi_zero`, `Phi_le_one`, `Phi_nonneg`) which become reusable infrastructure for any future Lean derivation invoking standard normal CDF facts."
  conditionalOn := []
...
def entry_prop_p_monotonicity : GapEntry where
  ...
  obstacleOrAttribution :=
    "PARTIAL bundle: (a) boundary κ*(2/3) = 0 CLOSED kernel-pure; (b) bounded-domain version `gap_p_monotonicity_bounded` CLOSED kernel-pure in R9; (c) universal `gap_p_monotonicity_OPEN` axiom is DEAD-END at axiom level (R9-falsified universal form via Lean junk-value semantics counterexample p₁=0, p₂=10). The bounded version is the live CLOSED Cat 1 sub-claim; the universal axiom should be renamed `gap_p_monotonicity_DEAD_END_by_junk_value` source-side (R17-D's responsibility)."
  conditionalOn := []

def entry_prop_bayesian_naive_five_state : GapEntry where
  name := "gap_bayesian_naive_routing_threshold + gap_bayesian_naive_reversal_{absent,present}_OPEN"
  status := GapStatus.gapPartial
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  ...
  obstacleOrAttribution := "PARTIAL bundle: routing-threshold sub-clause CLOSED kernel-pure (Cat 3 routing decision iff); reversal regimes (`absent`, `present`) remain OPEN axioms with substantive paper-novel content (Bayesian-naive 5-state reversal at p̂* = 2/3). Substantive proof requires bounded-rationality Bayesian-update derivation, Mathlib gap. R35-B Wave 2.9: Cat 2 chain to Blackwell 1951/1953 explicit on Part (ii) `gap_bayesian_naive_reversal_absent_OPEN` via the new `h_blackwell` antecedent (paper-APPLICATION-to-opaque-carrier per §10)."
  conditionalOn := []
...
def entry_rem_robustness_misspec_myopic_satisficing : GapEntry where
  name := "gap_robustness_{myopic_k,satisficing}_OPEN"
  status := GapStatus.gapOpen
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  ...
  attackHistory :=
    [ "R1 2026-05-12: 2 axioms with vacuous-Prop existentials.",
      ...
      "R11 discipline audit (2026-05-13): retroactive Ledger-status correction — the R7 source-side revert was applied but the Ledger entry was never updated, leaving stale `status := \"CLOSED\"` metadata that contradicted the actual Lean source (`Bayesian.lean:118/134` `axiom gap_robustness_{myopic_k,satisficing}_OPEN`) AND the AxiomAudit.lean:92 comment (\"`gap_robustness_myopic_k` and `gap_robustness_satisficing` are gap-OPEN axioms; see Ledger.\"). Status now corrected to OPEN, reconciling Ledger with both the source and AxiomAudit.",
      "R27-A 2026-05-13: Cat 3 sub-classification WORKING_ASSUMPTION per `feedback_gap_ledger_in_lean4` 2026-05-13 extension; status retained as OPEN (higher-level paper claim pending derivation from Cat 1 + Cat 2 + Cat 3 atomic inputs — 必须 close per discipline). New `subClass` field set to WORKING_ASSUMPTION." ]
  scope := "Remark rem:robustness-misspec (ii)+(iii), lines 942-944"
  obstacleOrAttribution :=
    "Substantive bounded-rationality content (k-step lookahead dynamics on the trap tree for myopic-k; satisficing-threshold acceptance criterion with non-monotone β-response) remains a Mathlib gap; Lean side encodes via opaque carriers `myopicKWelfare`, `satisficingWelfare`."
  conditionalOn := []
...
def entry_thm_general_tree : GapEntry where
  ...
  obstacleOrAttribution := "Substantive proof requires C2′ + non-interference + bounded-convergence."
  conditionalOn := []
...
def entry_ex_cyclic_trap : GapEntry where
  ...
  obstacleOrAttribution := "Cycle-trap example; opaque-carrier-bound substantive claim."
  conditionalOn := []

def entry_prop_error_compounding : GapEntry where
  name := "gap_error_compounding_part1 (CLOSED) + oracleValueAtRoot_TrapTree_def (Cat 3 atom) + gap_error_compounding_part2 (derived) + TrapTree.gap_welfare_gain_decay + gap_kappaStar_depth_d_log_growth_OPEN"
  status := GapStatus.gapPartial
  inputCategory := InputCategory.cat3PaperNovel
  cat3SubType := Cat3SubType.workingAssumption
  ...
  obstacleOrAttribution :=
    "Part 4 (welfare gain formula) CLOSED kernel-pure; Part 2 R23-C1 refactor: `oracleValueAtRoot_TrapTree_def` Cat 3 atomic structural-equation axiom + `gap_error_compounding_part2` derived theorem (CLOSED kernel-pure := the atom); Part 1 R30 promotion: `gap_error_compounding_part1` CLOSED via the Cat 1 helper chain shared with `gap_W_open_limit_infty` (signalVariance_tendsto_zero_atTop + tendsto_const_div_atTop_of_tendsto_zero_pos + tendsto_neg_atTop_atBot + Phi_tendsto_zero_atBot + continuous_pow). Parts 3 and 5 OPEN with substantive content. R24-D: AxiomAudit instrumentation now active on Part 2's derived closure."
  conditionalOn := []
...
def entry_atom_kappaStar_def : GapEntry where
  ...
  obstacleOrAttribution :=
    "Accepted as Cat 3 atomic axiom per discipline (extracted from bundled `gap_cognitive_threshold_part3_OPEN`). R28: WORKING_ASSUMPTION — pending derivation from paper IVT continuity inputs; 必须 close before publication."
  conditionalOn := []
...
def entry_atom_alphaStar_def : GapEntry where
  ...
  obstacleOrAttribution :=
    "Accepted as Cat 3 atomic axiom per discipline (paper-stated sup-characterisation on the existing `alphaStar` and `agentWelfare` carriers). R24-C: Option B classification — atomized stub awaiting downstream consumer (substantive sentimental-immunity closure expected to consume). R28: WORKING_ASSUMPTION — 必须 close before publication."
  conditionalOn := []
...
def entry_atom_betaBarStar_def : GapEntry where
  ...
  obstacleOrAttribution :=
    "Accepted as Cat 3 atomic axiom per discipline (argmax-characterisation on existing `betaBarStar` and `W_bar` carriers). R24-C: now consumed downstream by `W_bar_limit_infty_le_W_bar_betaBarStar` derived theorem (Principal.lean). R28: WORKING_ASSUMPTION — 必须 close before publication."
  conditionalOn := []
...
def entry_atom_W_bar_limit_infty_def : GapEntry where
  ...
  obstacleOrAttribution :=
    "Accepted as Cat 3 atomic axiom per discipline (paper-stated `Filter.Tendsto` characterisation on existing carriers). R24-C: now consumed downstream by `W_bar_limit_infty_le_W_bar_betaBarStar` derived theorem (Principal.lean). R28: WORKING_ASSUMPTION — 必须 close before publication."
  conditionalOn := []
...
def entry_atom_betaStarOfP_def : GapEntry where
  ...
  obstacleOrAttribution :=
    "Accepted as Cat 3 atomic axiom per discipline (argmin-characterisation on existing `betaStarOfP` and `L` carriers within Regime (i)'s domain). R24-C: now consumed downstream by `betaStarOfP_loss_below_limit` derived theorem (Canonical.lean). R28: WORKING_ASSUMPTION — 必须 close before publication."
  conditionalOn := []
