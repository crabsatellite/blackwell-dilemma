...
/-- **Theorem 6.1** (`thm:general-tree`) — reversal-witness
    decomposition.

    Reversal-witness decomposition: the single-atom
...
def agentRewardKernel_greedy_C2prime_kernel_reversal_witness : Prop :=
...
/-- Current scalar-kernel closure of the C2′ greedy reversal witness. The
    diagnostic condition remains in the interface, but the present
    `agentRewardKernel` supplies the β-pair `(0, 1)` at α = 1. -/
theorem agentRewardKernel_greedy_C2prime_kernel_reversal_witness_current :
    agentRewardKernel_greedy_C2prime_kernel_reversal_witness := by
  intro _hC
...
/-- Generic **Theorem 6.1** (`thm:general-tree`) route — derived theorem via
    an explicit reversal-witness package.
    Subsumes Theorem 3.2 via `dilemma_subsumed_by_gap_general_tree`
    (terminal-neighbour topology + C2 ⇒ C2′).

    Closure composes (a) the explicit kernel
    reversal-witness interface
    ...
    paper source: Theorem 6.1 (`thm:general-tree`), lines 989-998. -/
theorem gap_general_tree_from_reversal
    (h_reversal : agentRewardKernel_greedy_C2prime_kernel_reversal_witness) :
    Conditions_C1_C2prime_C3 →
    ∃ β β' : ℝ, β < β' ∧
...
/-- Current scalar-kernel wrapper for Theorem 6.1, with the C2′ greedy
    reversal witness discharged by
    `agentRewardKernel_greedy_C2prime_kernel_reversal_witness_current`. -/
theorem gap_general_tree_current :
    Conditions_C1_C2prime_C3 →
    ∃ β β' : ℝ, β < β' ∧
      agentWelfare AgentType.greedy β' 0 1 <
        agentWelfare AgentType.greedy β 0 1 :=
  gap_general_tree_from_reversal agentRewardKernel_greedy_C2prime_kernel_reversal_witness_current

/-- Public current scalar-kernel wrapper for Theorem 6.1. The generic route is
    `gap_general_tree_from_reversal`; the public theorem consumes the current
    greedy reversal witness internally. -/
...
theorem gap_general_tree_current_of_fin5Trap_under_parametricLocalC2primeData
    {c3 : Prop}
    {isBlackwellOrdered : (ℝ → PercolationOutcome → ℝ) → Prop}
    (hC3 : c3) :
    letI : DiagnosticSignalHypothesisData :=
  ...
  letI : DiagnosticSignalHypothesisData :=
    diagnosticSignalHypothesisDataWithParametricLocalC2prime
      c3 isBlackwellOrdered
  exact gap_general_tree_current
    (Conditions_C1_C2prime_C3_of_fin5Trap_under_parametricLocalC2primeData
      hC3)

theorem gap_general_tree_current_of_fin5Trap_under_parametricLocalC2primeConcreteC3Data
    {isBlackwellOrdered : (ℝ → PercolationOutcome → ℝ) → Prop} :
    letI : DiagnosticSignalHypothesisData :=
      diagnosticSignalHypothesisDataWithParametricLocalC2primeAndConcreteC3
        isBlackwellOrdered
  ...
  letI : DiagnosticSignalHypothesisData :=
    diagnosticSignalHypothesisDataWithParametricLocalC2primeAndConcreteC3
      isBlackwellOrdered
  exact gap_general_tree_current
    Conditions_C1_C2prime_C3_of_fin5Trap_under_parametricLocalC2primeConcreteC3Data

theorem gap_general_tree_current_of_fin5Trap_under_parametricLocalC2primeConcreteC3NoBlackwellData :
    letI : DiagnosticSignalHypothesisData :=
      diagnosticSignalHypothesisDataWithParametricLocalC2primeConcreteC3NoBlackwell
    ∃ β β' : ℝ, β < β' ∧
      agentWelfare AgentType.greedy β' 0 1 <
  ...
  letI : DiagnosticSignalHypothesisData :=
    diagnosticSignalHypothesisDataWithParametricLocalC2primeConcreteC3NoBlackwell
  exact gap_general_tree_current
    Conditions_C1_C2prime_C3_of_fin5Trap_under_parametricLocalC2primeConcreteC3NoBlackwellData

def gap_dilemma_current_of_fin5Trap_parametricLocalGreedy :
    (∃ beta : Real,
      ∃ beta' : Real,
        beta < beta' ∧
          agentWelfare AgentType.greedy beta' 0 1 <
      ...
      (∃ data : WrongnessPercolationData,
        WInfoOracleInterfacesOn data ∧
          OracleInfoNonzeroWitnessOn data) := by
  exact ⟨
    gap_general_tree_current_of_fin5Trap_under_parametricLocalC2primeConcreteC3NoBlackwellData,
    currentOracleInfoDecayConclusion⟩

theorem fin5Trap_parametricDilemmaCurrentBridge_viaGeneralTree :
    ParametricDilemmaCurrentBridge := by
  exact ⟨fin5Trap_parametricDilemmaGraphScopeWitness,
    fin5Trap_parametricLocalC2primeFullWitness,
    gap_dilemma_current_of_fin5Trap_parametricLocalGreedy⟩
...
/-- **Example `ex:cyclic-trap`** (closure-path-A derived theorem
    ...
    `Cyclic4BlockedEventConditions`
    (paper line 1028 — diagnostic conjunction holds at the blocked
    event) with `gap_general_tree_from_reversal` (generic derived theorem via
    reversal-witness pattern; paper Theorem 6.1 —
    `Conditions_C1_C2prime_C3` ⇒ greedy β-reversal exists).
    Non-monotonicity survives on graphs with cycles.

    The encoding uses a strictly-smaller atom (just the
    diagnostic-conjunction validity at the blocked event) chained with
    the Theorem 6.1 atom rather than a bundled
    `cyclic_4_satisfies_C2prime_at_open_event` packaging both the
    diagnostic-conjunction validity AND the existential β-reversal
    conclusion of Theorem 6.1. The atomic decomposition exposes the
    paper-stated structural composition explicitly: the cyclic-trap
    example satisfies the Theorem 6.1 hypotheses, hence inherits the
    Theorem 6.1 conclusion.

    paper source: Example `ex:cyclic-trap`, lines 1026-1029. -/
theorem gap_cyclic_trap_from_reversal
    (h_reversal : agentRewardKernel_greedy_C2prime_kernel_reversal_witness)
    (hC2prime : C2prime_GreedyPathMisalignment)
    (hC3 : C3_InformationLocality) :
    ∀ p : ℝ, 0 < p → p < 1 →
  ...
  exact gap_general_tree_from_reversal h_reversal
    (cyclic_4_satisfies_full_conditions_at_blocked_event_closed
      hC2prime hC3 p hp_pos hp_lt_one)

/-- Public current scalar-kernel wrapper for Example `ex:cyclic-trap`. The
    greedy reversal witness is discharged by the current kernel theorem; C2′
    and C3 remain explicit diagnostic-scope hypotheses. -/
