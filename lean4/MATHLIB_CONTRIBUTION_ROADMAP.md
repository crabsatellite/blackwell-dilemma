...
positive flat-loss witness, the same family-core theorem package, and
non-diagnostic guards excluding the current full-reach, flat-only,
all-open-complement, and pointwise-hybrid diagnostic families;
`PaperSemanticGate.lean` gates its projections and exclusions so the eventual
